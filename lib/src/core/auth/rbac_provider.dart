  ...
  Future<bool> loginWithPin(String pin) async {
    try {
      ...
      if (staff.roleId != null) {
        role = await (_db.select(
          _db.roles,
        )..where((t) => t.id.equals(staff.roleId!))).getSingleOrNull();
      }

      final permissions = <Permission>{};
      if (role?.canRefund == true) permissions.add(Permission.refund);
      if (role?.canVoid == true) permissions.add(Permission.voidSale);
      if (role?.canPriceOverride == true) {
        permissions.add(Permission.priceOverride);
      }
